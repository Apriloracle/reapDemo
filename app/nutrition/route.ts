...
const MANIFEST = {
  ...
  accepts: [
    {
      ...
      resource: 'https://reap.deals/nutrition',
      description: 'Get nutrition data for a given product.',
        ...
        input: {
          ...
          bodyFields: {
            productDetails: {
              type: 'object',
              required: true,
              description: 'Product details to search for.',
            },
          },
        },
        output: {
  ...
  meta: {
    name: 'Nutrition Data',
    description: 'Real-time nutrition data for products',
...
export async function POST(request: Request) {
  ...
  let productDetails: object | undefined;
  try {
  ...
  if (!productDetails) {
    return new NextResponse(
      JSON.stringify({ error: 'A "productDetails" field is required in the request body.' }),
      { status: 400, headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' } }
    );
  }

  try {
    const agentEndpoint = 'https://claim.reap.deals/nutrition';
