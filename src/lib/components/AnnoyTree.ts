...
export class AnnoyTree {
    ...
    private values: DataPoint[] = []; // Initialize as an empty array

    private isLeaf: boolean = true; // Initialize as a leaf node
    ...
    private validatePoint(p: Vector): void {
        ...
        if (p.length !== this.dimensions) {
            throw new Error(`Failed to 'get()'. A vector of ${p.length} dimensionality was provided. Please provide a vector of ${this.dimensions} dimensionality`);
        }
    }
    ...
    private setAsInnerNode(dividingHyperplane: Hyperplane = this.genDividingHyperplane(), rightTree: AnnoyTree | null = null, leftTree: AnnoyTree | null = null): void {
        ...
        this.values = []; // Clear the values when becoming an inner node
    }
    ...
    private genDividingHyperplane(): Hyperplane {
        if (!this.values || this.values.length === 0) {
            throw new Error("Cannot generate dividing hyperplane: no values in the node");
        }
        ...
        return new Hyperplane(centroid);
    }
